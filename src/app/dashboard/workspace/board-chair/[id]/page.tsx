...
import {
  getKnownBoardChairDocs, loadLocalBoardChairDocs,
...
export default function BoardChairDocViewerPage() {
  const params = useParams()
  const router = useRouter()
  const [doc, setDoc] = useState<BoardChairDocument | null>(null)

  useEffect(() => {
    const all = [...loadLocalBoardChairDocs(), ...getKnownBoardChairDocs()]
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 64px)' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={() => router.push('/dashboard/workspace/board-chair')}
            style={{
