...
import {
  ShieldCheck, MagnifyingGlass, FileText, Scales, Buildings,
  CurrencyCircleDollar, Users, Leaf, ArrowRight, PenNib, CheckCircle,
...
export default function ValidatedDocumentsPage() {
  ...
  useEffect(() => { setFinalisedDocs(loadFinalisedDocuments()) }, [])
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} style={{ marginBottom: 32 }}>
        ...
        <h1 className="text-gold font-display font-black" style={{ fontSize: 32, marginBottom: 6 }}>Validated Documents Vault</h1>
        <p className="text-text-secondary" style={{ fontSize: 15, maxWidth: 600 }}>
          Approved corporate documents from the Socinga Africa R2 archive. Click any document to view, edit inline, digitally sign, and export.
        </p>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(10,17,40,0.6)', border: '1px solid rgba(212,175,55,0.2)', padding: '8px 14px', flex: '1 1 300px', maxWidth: 400 }}>
          <MagnifyingGlass size={16} className="text-gold" />
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search validated documents..."
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {CATEGORIES.map(c => (
            <button key={c} onClick={() => setCat(c)} style={{
              padding: '5px 12px', fontSize: 11, fontWeight: 600, fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.5px',
